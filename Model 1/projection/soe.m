%% Solves SOE model using projection methods
...
while ssr>err_tol
    ...
    b_p = exp( z_mesh ) + r * b_mesh -  ( 1 + chi ) ./ ( mu_p - delta * b_p ); 
    mu_p = ex_mu( b , z , b_p , params );
    error = up( ( exp( z_mesh ) + r * b_mesh - b_p ) / (1 + chi) ) - mu_p + delta * b_p;
    ssr = sum( sum( error.^2 ) );
...
projection.irf_unbounded.c.epsz = mean(cons_mat(:,irf_drop+1:end));
projection.irf_unbounded.b.epsz = mean(bonds_mat(:,irf_drop+1:end));
projection.irf_unbounded.h.epsz = mean(hours_mat(:,irf_drop+1:end));
